...
To disable caching for a specific client, pass `disable_prompt_cache=True`:
...
```typescript TypeScript
import * as hub from "langchain/hub";
import { configureGlobalPromptCache, promptCacheSingleton } from "langsmith";
...
prompt_cache_singleton.get("joke-generator:latest")
...
Note: The background refresh task is only started when you first set a value in the cache, and only if `ttl_seconds` is not `None`. If `ttl_seconds` is `None` (offline mode), no background task is created.
